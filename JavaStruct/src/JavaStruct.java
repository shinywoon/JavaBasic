...
//        }
		...
		// 검색을 위한 자료구조
		// key 를 이용하여 값을 저장하고 key를 이용하여 값을 꺼내오는 방식 - hash 알고리즘으로 구현됨
		// key가 되는 객체는 중복될 수 없고 객체의 유일성을 비교를 위한 equals()와 hashCode()매서드를 구현해야 함
		...
		// containsKey 메서드를 사용해서 Map에 해당 key가 있는지 확인한 뒤 결과값을 리턴한다.

		// remove
		// remove 메서드는 Map의 항목을 삭제하는 메서드로 key에 해당하는 아이템을 삭제한 후 그 value 값을 리턴한다.
		System.out.println(map.remove("num2"));
		...
		// Map에 입력된 순서대로 데이터를 가져오고 싶을 때는 LinkedHashMap을 사용하면 되고

		// 입력된 Key의 오름차순 순서로 데이터를 가져오고 싶을 때는 TreeMap을 사용하면 된다.
